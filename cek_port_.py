...
def run(ip, port):
    ...
    if(txt == "ya"):
        txt_again = input("apakah hasil uji mau disimpan? (ya/tidak) : ")
        if (txt_again == "ya"):
            txt_pilih = input("format xlsx atau text:(xlsx/text) ")
            if(txt_pilih == "text"):
                try:
            ...
            if(txt_pilih == "xlsx"):
                try:
                    nama_file_xlsx= input("tulikan nama file yang akan jadi report? contoh (report.xlsx) : ")
                    report_xlsx(all_port,nama_file_xlsx)
                except:
                    print("nama file harus diakhiri dengan .xlsx")
